...
function removeFirstOccurrences(str, value) {
  const idxSubStr = str.indexOf(value);

  if (idxSubStr === -1) return str;

  return str.slice(0, idxSubStr) + str.slice(idxSubStr + value.length);
}
...
function removeLastOccurrences(str, value) {
  const idxSubStr = str.lastIndexOf(value);
  if (idxSubStr === -1) return str;
  return str.slice(0, idxSubStr) + str.slice(idxSubStr + value.length);
}
...
function sumOfCodes(str) {
  const isStr = str === undefined || str === '' || str === null;
  if (isStr) return 0;
  const arrStr = str
    .split('')
...
function formatTime(minutes, seconds) {
  const minutesRes =
    minutes >= 0 && minutes < 10 ? String(minutes).padStart(2, '0') : minutes;
  const seconsRes =
    seconds >= 0 && seconds < 10 ? String(seconds).padStart(2, '0') : seconds;
  return `${minutesRes}:${seconsRes}`;
}
...
function countVowels(str) {
  const arr = str.split('');
  const lengthStr = arr.filter((letter) =>
    'aeiouyAEIOUY'.includes(letter)
  ).length;
  return lengthStr;
}
...
function isPalindrome(str) {
  const re = /[\W_]/g;
  const strUpperCase = str.replaceAll(re, '').toUpperCase();
  const arr = strUpperCase.split('');
  const strReverse = arr.reverse('').join('');
  return strUpperCase === strReverse;
}
